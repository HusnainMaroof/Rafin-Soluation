  ...
  return (
    <footer className="bg-slate-900 text-white pt-24 pb-0 relative z-50">
      <div className="container mx-auto px-6 lg:px-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-20 relative z-10">
        ...
        <div>
          <h4 className="text-xs font-heading tracking-widest text-slate-500 uppercase mb-6">
            Contact Info
          </h4>
              ...
              {FOOTER_LINKS.quickLinks.map((link) => (
                <li key={link.title}>
                  <Link href={link.href} key={link.title}>
                    <span className="hover:text-yellow-400 transition-colors inline-block py-1">
                      {link.title}
                    </span>
                  </Link>
                </li>
